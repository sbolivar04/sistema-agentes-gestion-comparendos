...
class ClienteNavegadorSimit:
    ...
    async def _consultar_criterio_en_pagina(
        ...
        """
        Ejecuta la consulta de un NIT o Placa en una página ya abierta de SIMIT,
        modificando únicamente la caja de búsqueda superior (#txtBusqueda) sin recargar la página.
        """
        ...
        if not busqueda_exitosa:
            error_msg = f"SIMIT no respondió correctamente tras {max_intentos} intentos para {criterio_clean}."
        ...
        if es_vacio:
            logger.info(f"Extracción finalizada (0 comparendos para {criterio_clean}).")
            return ResultadoConsultaSchema(
                criterio_busqueda=criterio_clean,
                tipo_consulta=TipoConsulta.NIT if tipo_consulta == "NIT" else TipoConsulta.PLACA,
                exitoso=True,
        ...
        # EXTRACCIÓN ROBUSTA EN DOS PASOS: TABLA PRINCIPAL Y VISTA DETALLADA
        ...
        comparendos_enriquecidos = []
        for comp in comparendos_extraidos:
            comp_actualizado = calculate_discounts(comp)
            comparendos_enriquecidos.append(comp_actualizado)
