...
from agente_orquestador.herramientas import obtener_y_limpiar_evento_extraccion
...
class SolicitudMensajeChat(BaseModel):
    ...
    modelo: Optional[str] = Field(None, description="Modelo a utilizar")

# Instancia singleton del agente para el chat web
_instancia_agente = None

def _obtener_agente(modelo: Optional[str] = None):
    global _instancia_agente
    modelo_final = modelo or configuracion.MODELO_GEMINI
    if _instancia_agente is None or _instancia_agente.modelo != modelo_final:
        _instancia_agente = AgenteOrquestadorComparendos(modelo=modelo_final)
...
def procesar_mensaje_chat(solicitud: SolicitudMensajeChat) -> Dict[str, Any]:
    ...
    notificando si se inició una sincronización en vivo con el SIMIT.
    """
    try:
        agente = _obtener_agente(solicitud.modelo)
        respuesta = agente.procesar_mensaje(solicitud.mensaje)
        evento_extraccion = obtener_y_limpiar_evento_extraccion()
        
        return {
            "exitoso": respuesta.exitoso,
            "respuesta": respuesta.texto_respuesta,
            "error": respuesta.mensaje_error,
            "extraccion_iniciada": bool(evento_extraccion),
            "detalles_extraccion": evento_extraccion
        }
